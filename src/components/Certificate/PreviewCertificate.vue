<template>
  ...
  <SectionSpace>
    <div class="row mb-3 me-1">
      ...
      <BaseButton class="col-auto ms-auto" label="ดาวน์โหลดทั้งหมด">
        <template #before-text>
          <download class="custom-icon me-2" />
        </template>
      </BaseButton>
    </div>
...
const getAllIntern = async () => {
  await axios
    .get(`${import.meta.env.VITE_API_HOST}/interns?almost=${date}&noCertificate=true`, {
      params,
    })
    .then((response) => {
